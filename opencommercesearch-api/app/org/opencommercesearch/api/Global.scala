...
object Global extends WithFilters(new StatsdFilter(), new GzipFilter(), AccessLog) {
  ...
  lazy val MaxPaginationLimit = getConfig("products.pagination.limit.max", 60)
  lazy val MaxBrandPaginationLimit = getConfig("brand.pagination.limit.max", 3000)
  ...
  lazy val SolrErrorRetry = getConfig("solr.error.retry", 3);
