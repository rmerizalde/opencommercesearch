...
object Util {
  ...
  /**
   * Get accepted language from request.
   * @param req AnyContent request.
   */
  def getLanguageFromRequest(req : Request[AnyContent]) : String = {
  ...
  /**
   * Get accepted language from request.
   * @param req JsValue request.
   */
  def getLanguageFromJSRequest(req : Request[JsValue]) : String = {
    var language: String = "en"
    req.acceptLanguages.map(lang => language = lang.language)
    language
  }
