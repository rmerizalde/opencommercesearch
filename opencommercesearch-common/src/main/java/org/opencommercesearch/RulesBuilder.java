...
public class RulesBuilder extends GenericService {
    ...
    enum Rule {
        PCT_OFF {
            public String toFilter(String ruleValue, Locale locale) {
                return new StringBuilder().append("discountPercent").append(locale.getCountry()).append(":[")
                        .append(ruleValue).append(" TO 100]").toString();
            }
        },
        CATEGORY {
            public String toFilter(String ruleValue, Locale locale) {
        ...
        PRICE {
            public String toFilter(String ruleValue, Locale locale) {
                ...
                String min = parts[0];
                String max = parts[1];
                return new StringBuilder().append("salePrice").append(locale.getCountry()).append(":[").append(min)
                        .append(" TO ").append(max).append("]").toString();
            }
        },
        KEYWORD {
            public String toFilter(String ruleValue, Locale locale) {
                return new StringBuilder().append("title:").append(ClientUtils.escapeQueryChars(ruleValue)).toString();
            }
        };

        public String toFilter(RepositoryItem ruleItem, Locale locale) {
            String value = (String) ruleItem.getPropertyValue(RuleExpressionProperty.VALUE);
            value = value.trim();
            if (value.endsWith(",")) {
                value = value.substring(0, value.length() - 1);
            }
            return toFilter(value, locale);
        }

        public abstract String toFilter(String ruleValue, Locale locale);
    }
    ...
    private String buildFilter(RepositoryItem category, List<RepositoryItem> ruleExpressions, Locale locale) {
        ...
        if (ruleExpressions.size() == 0) {
            filter.append(Rule.CATEGORY.toFilter(category.getRepositoryId(), locale)).append(")");
        ...
        for (RepositoryItem ruleItem : ruleExpressions) {
            ...
            String ruleString = (rule.toFilter(ruleItem, locale));
