...
module.exports = (function ProductApi(params) {
  ...
  var setEndpoint = api.setEndpoint = function(name, endpoint, override) {
    if (!endpoint && !endpoint.tpl) {
      throw new Error(moduleName + 'endpoint needs a template');
    }

    if (!api[name] || override) {
      api[name] = function(request, options) {
        var req = helpers.buildRequest(endpoint, request, options);

        return helpers.apiCall(req);
      };
      return true;
    } else {
      throw new Error(moduleName + 'endpoint ' + name + ' already exists');
    }
  };
  ...
  var helpers = {
    ...
    buildOptions: function(defaults, options) {
      ...
      // productIds don't need to be passed
      delete options.productId;
      return options;
    },
    buildRequest: function(endpoint, requestParams) {
      if (typeof requestParams !== 'object') {
        throw new Error(moduleName + 'request for ' + endpoint.tpl + ' must be an object');
      }
